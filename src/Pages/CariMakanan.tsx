...
const CariMakanan = () => {
  ...
  const isOrderableTime = useCallback((item: FoodItem): boolean => {
  ...
  const applyFilters = useCallback(() => {
    ...
    tempItems.sort((a, b) => {
      const isA_Available = a.current_stock > 0 && isOrderableTime(a);
      const isB_Available = b.current_stock > 0 && isOrderableTime(b);

      if (isA_Available === isB_Available) {
        return 0;
      }
      return isA_Available ? -1 : 1;
    });

    setFilteredFoodItems(tempItems);
  }, [allFoodItems, categoryFilter, priceFilter, distanceFilter, isOrderableTime]);
