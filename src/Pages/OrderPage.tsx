...
const OrderItem: React.FC<OrderItemProps> = ({ order, isHistory, onCancel, onReview, onDelete }) => {
        ...
        <div className="bg-white p-4 rounded-lg shadow-md border flex flex-col md:flex-row justify-between items-start gap-4">
            ...
            <div className="flex-grow">
                ...
                <p className="text-xs text-gray-500 mt-1">
                    {new Date(order.order_date).toLocaleString('id-ID', { dateStyle: 'long', timeStyle: 'short' })}
                </p>
            </div>
            <div className="flex flex-col space-y-2 w-full md:w-auto self-center md:items-end">
                <span className={`px-2 py-1 rounded-full text-xs font-medium self-start md:self-end ${getStatusBadge(order.status)}`}>
                    {order.status.replace(/_/g, ' ')}
                </span>
                <div className="flex-grow" />
                <div className="flex flex-row md:flex-col gap-2 w-full">
                    {order.status === 'pending' && <button onClick={() => onCancel(order.id)} className="flex-1 bg-red-500 text-white px-3 py-1.5 rounded-md hover:bg-red-600 transition text-sm">Batalkan</button>}
                    {order.status === 'sudah_diambil' && !order.ulasan && <button onClick={() => onReview(order)} className="flex-1 bg-blue-500 text-white px-3 py-1.5 rounded-md hover:bg-blue-600 transition text-sm">Beri Ulasan</button>}
                    {order.ulasan && <div className="text-xs text-center p-2 bg-gray-100 text-gray-600 rounded-md w-full">Sudah Diulas</div>}
                    {isHistory && <button onClick={() => onDelete(order.id)} className="flex-1 bg-gray-500 text-white px-3 py-1.5 rounded-md hover:bg-gray-600 transition text-sm">Hapus</button>}
                </div>
            </div>
        </div>
    );
...
const OrderPage = () => {
    ...
    const handleCancelOrder = async (orderId: number) => {
        if (!window.confirm("Apakah Anda yakin ingin membatalkan pesanan ini?") || !token) return;
        try {
            ...
            if (!response.ok) throw new Error(result.message || 'Gagal membatalkan pesanan.');
    ...
    const handleDeleteOrder = async (orderId: number) => {
        if (!window.confirm("Tindakan ini akan menghapus riwayat pesanan dari tampilan. Lanjutkan?")) return;
    ...
    const renderOrderList = (orderList: PesananType[], isHistory: boolean) => {
        if (isLoading) {
            return <div className="space-y-4">{[...Array(3)].map((_, i) => <OrderSkeleton key={i} />)}</div>;
        }
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="border-b border-gray-200 mb-6">
                <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                    ...
                    <button 
                        ...
                        className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'history' ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                    >
            ...
            <div>
                {activeTab === 'active' ? renderOrderList(activeOrders, false) : renderOrderList(historyOrders, true)}
            </div>
